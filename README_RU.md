...
- **💬 Генерация текста (t2t)**: Все текстовые модели Qwen (streaming, ~2-5 сек)
- **🖼️ Генерация изображений (t2i)**: Text-to-Image с автоматическим streaming (~10-20 сек)
- **🎬 Генерация видео (t2v)**: Text-to-Video с автоматическим опросом задачи (~30-60 сек)
- **🔄 Управление несколькими аккаунтами**: Автоматическое переключение токенов
- **🔌 Совместимость с OpenAI**: Поддержка OpenAI SDK и API формата
- **📂 Загрузка файлов**: Загрузка изображений через прокси на Qwen CDN
...
- [🎨 Генерация изображений и видео](#-генерация-изображений-и-видео)
...
1. **Файл `src/Authorization.txt`**
   - Создаётся автоматически при первом запуске _если его нет_.
...
- **🎨 Генерировать изображения из текстовых описаний (Text-to-Image)**
- **🎬 Генерировать видео из текстовых описаний (Text-to-Video)**
...
Прокси поддерживает **генерацию изображений (t2i)** и **генерацию видео (t2v)** через параметр `chatType`.

### Сравнение типов генерации

| Характеристика            | Текст (t2t)           | Изображение (t2i)            | Видео (t2v)                  |
| ------------------------- | --------------------- | ---------------------------- | ---------------------------- |
| **Формат ответа**         | Потоковый (streaming) | Потоковый (streaming)        | Опрос задачи (task polling)  |
| **Время генерации**       | ~2-5 сек              | ~10-20 сек                   | ~30-60 сек                   |
| **URL в поле**            | N/A                   | `content`                    | `content` и `video_url`      |
| **Параметр `stream`**     | `true` (по умолчанию) | `true` (по умолчанию)        | `false` (автоматически)      |
| **Формат размера**        | N/A                   | Соотношение сторон: `"16:9"` | Соотношение сторон: `"16:9"` |
| **Рекомендуемый таймаут** | 30 сек                | 60 сек                       | 180 сек                      |
...
POST /api/chat
{
  "message": "Красивый закат над океаном",
...
POST /api/chat
{
  "message": "Океанские волны, плавно накатывающие на пляж",
  "model": "qwen-vl-max-latest",
  "chatType": "t2v",
  "size": "16:9",
  "waitForCompletion": true  // false для опроса на стороне клиента
}
```

**Режимы опроса:**
- `waitForCompletion: true` (по умолчанию) - Сервер опрашивает, возвращает готовый URL видео
- `waitForCompletion: false` - Возвращает task_id немедленно для опроса на стороне клиента

**Важно:** Формат размера использует **соотношение сторон** (например, `"16:9"`), а не размеры в пикселях.

### Формат ответа

Оба возвращают URL на сгенерированный контент:
...
```json
{
  "choices": [
    {
      "message": {
        "content": "https://cdn.qwenlm.ai/output/.../image.png"
      }
    }
  ],
  "chatId": "...",
...
**📖 Подробная документация: [IMAGE_VIDEO_GENERATION_GUIDE.md](IMAGE_VIDEO_GENERATION_GUIDE.md)**

- Полный справочник параметров
- Форматы ответов и детали опроса
...
| `/api/tasks/status/:taskId` | GET   | Проверка статуса задачи генерации видео (ручной опрос)      |
...
_Причина: чаты теперь управляются на серверах Qwen_
    ...
    messages: [
      {
        role: "system",
        content:
          "Ты - опытный программист на Python. Отвечай кратко и предоставляй примеры кода.",
      },
      {
        role: "user",
...
```json
{
  "messages": [
    {
      "role": "system",
      "content": "Ты эксперт по JavaScript. Отвечай только на вопросы о JavaScript."
    },
